...
export default function Home() {
  const [list, setList] = useState<DishType[]>(() => {
    if (typeof window !== 'undefined') {
      ...
      return savedList ? JSON.parse(savedList) : foods.map((item, index) => ({ id: index, name: item.name, type: item.type, checked: true }));
    }
    return foods.map((item, index) => ({ id: index, name: item.name, type: item.type, checked: true }));
  });
  ...
  const isEmpty = useMemo(() => list.filter(item => item.checked).length === 0, [list]);
  const [menuList, setMenuList] = useState<string[]>(list.map(item => item.name));
